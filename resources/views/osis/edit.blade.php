                    ...
                    <form action="{{ route('osis.update', $osis->id) }}" method="POST">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-control-lg" id="siswa_id" name="siswa_id">
                                    ...
                                    @foreach ($siswas as $siswa)
                                        <option value="{{ $siswa->id }}" {{ $osis->siswa_id == $siswa->id ? 'selected' : '' }}>
                                            {{ $siswa->user->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-control" id="jabatan" name="jabatan" required>
                                    ...
                                    <option value="Ketua OSIS" {{ $osis->jabatan == 'Ketua OSIS' ? 'selected' : '' }}>Ketua OSIS</option>
                                    <option value="Wakil Ketua OSIS" {{ $osis->jabatan == 'Wakil Ketua OSIS' ? 'selected' : '' }}>Wakil Ketua OSIS</option>
                                    <option value="Sekretaris" {{ $osis->jabatan == 'Sekretaris' ? 'selected' : '' }}>Sekretaris</option>
                                    <option value="Bendahara" {{ $osis->jabatan == 'Bendahara' ? 'selected' : '' }}>Bendahara</option>
                                    <option value="Anggota" {{ $osis->jabatan == 'Anggota' ? 'selected' : '' }}>Anggota</option>
                                </select>
                                @error('jabatan')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" id="periode" name="periode" value="{{ $osis->periode }}" required>
                                @error('periode')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
