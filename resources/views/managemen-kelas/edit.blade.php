...
@section('content')
    ...
    <div class="row layout-top-spacing">
        <div class="row mb-2">
            <div class="col-12">
                <a href="/kesiswaan/kelas" class="btn btn-primary mr-2">Kembali</a>
                    ...
                    <form action="{{ route('kelas.update',$kelas->id) }}" method="POST">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" placeholder="Isikan Nama Kelas" value="{{ $kelas->nama_kelas }}" class="form-control" id="nama_kelas" name="nama_kelas" required>
                                @error('nama_kelas')
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-control-lg" id="wali_kelas_id" name="wali_kelas">
                                    ...
                                    @foreach ($pendidikTendiks as $guru)
                                        <option value="{{ $guru->id }}" @if ($kelas->wali_kelas_id == $guru->id) selected @endif>{{ $guru->user->name }}</option>
                                    @endforeach
                                </select>
                                @error('wali_kelas')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
